...
class PostureProcessor {
  ...
  static const double gForce = 9.807;
  static const double defaultHeight = 165.0;
  ...
  static void _computeFromDataImpl(Map<String, Object> args) {
    ...
    final accXWithG = data.map((e) => e.accelerometerX / gForce);
    final accYWithG = data.map((e) => e.accelerometerY / gForce);
    final accZWithG = data.map((e) => e.accelerometerZ / gForce);
...
Matrix rotateAxis(List<double> accX, List<double> accY, List<double> accZ) {
  ...
  if (!crossVector.all((double) => double == 0.0)) {
    ...
    final rMatrix = (Matrix.identity(3) + zMatrix + matrix3x3pow2(zMatrix) * ((1 - dot) / pow(crossVector.norm(), 2))) / pow(p0Norm, 2);
